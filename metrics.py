...
def record_conversation(scenario: str, action: str, cost: float,
                        tiers: list[str], holdout: bool = False) -> None:
    _M["conversations"] += 1
    _M["by_scenario"][scenario] += 1
    _M["by_action"][action] += 1
    _M["llm_cost_usd"] = round(_M["llm_cost_usd"] + cost, 6)
    for t in tiers:
        _M["by_model_tier"][t] += 1
    if holdout:
        _M["holdout_conversations"] += 1

    if scenario not in NON_ADDRESSABLE:
        _M["addressable_total"] += 1
        if action in DEFLECTING_ACTIONS:
            _M["addressable_deflected"] += 1
    if action in DEFLECTING_ACTIONS:
        _M["deflected"] += 1
    if action == Action.ESCALATE_HUMAN.value:
        _M["escalations"] += 1
    if action == Action.INSTANT_REFUND.value:
        _M["instant_refunds"] += 1
...
def snapshot() -> dict:
    ...
    return {
        ...
        "routing": dict(_M["by_model_tier"]),
        "scenarios": dict(_M["by_scenario"]),
        "actions": dict(_M["by_action"]),
        "guardrail_trips": dict(_M["guardrail_trips"]),
